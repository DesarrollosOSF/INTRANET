    ...
    if ($accion === 'crear' || $accion === 'editar') {
        ...
        if (empty($nombre) || $dependencia_id <= 0) {
            ...
            if ($mensaje === '') {
                try {
                    if ($accion === 'crear') {
                        $stmt = $pdo->prepare("INSERT INTO sst_documentos (nombre, dependencia_id, descripcion, imagen, usuario_id) VALUES (?, ?, ?, ?, ?)");
                        $stmt->execute([$nombre, $dependencia_id, $descripcion ?: null, $imagen, $_SESSION['usuario_id']]);
                        $mensaje = 'Documento SST creado. Ahora puede agregar módulos y archivos en el detalle.';
                        $tipo_mensaje = 'success';
                        registrarLog($_SESSION['usuario_id'], 'Crear documento SST', 'SST', "Documento: $nombre");
                    } else {
                        $doc_actual = $pdo->prepare("SELECT imagen FROM sst_documentos WHERE id = ?");
                        $doc_actual->execute([$id]);
                        $row = $doc_actual->fetch();
                        $imagen_final = $imagen ?: ($row['imagen'] ?? null);
                        $stmt = $pdo->prepare("UPDATE sst_documentos SET nombre = ?, dependencia_id = ?, descripcion = ?, imagen = COALESCE(?, imagen) WHERE id = ?");
                        $stmt->execute([$nombre, $dependencia_id, $descripcion ?: null, $imagen, $id]);
                        $mensaje = 'Documento actualizado.';
                        $tipo_mensaje = 'success';
                        registrarLog($_SESSION['usuario_id'], 'Editar documento SST', 'SST', "ID: $id");
                    }
        ...
        $id = (int)$_POST['id'];
        $stmt = $pdo->prepare("SELECT imagen FROM sst_documentos WHERE id = ?");
        $stmt->execute([$id]);
        $row = $stmt->fetch();
        $pdo->prepare("DELETE FROM sst_documentos WHERE id = ?")->execute([$id]);
        if ($row && $row['imagen'] && file_exists(BASE_PATH . 'uploads/' . $row['imagen'])) {
            @unlink(BASE_PATH . 'uploads/' . $row['imagen']);
        }
        $mensaje = 'Documento eliminado.';
        $tipo_mensaje = 'success';
        registrarLog($_SESSION['usuario_id'], 'Eliminar documento SST', 'SST', "ID: $id");
    }
}
...
try {
    $count_sql = "SELECT COUNT(*) FROM sst_documentos d INNER JOIN dependencias dep ON dep.id = d.dependencia_id";
...
try {
    $documentos = $pdo->query("
        SELECT d.*, dep.nombre AS dependencia_nombre,
               (SELECT COUNT(*) FROM modulos_sst m WHERE m.sst_documento_id = d.id) AS num_modulos
    ...
    try {
        ...
        error_log('SST listado: ' . $e2->getMessage());
        $documentos = [];
    }
}
...
<div class="container-fluid mt-4 pagina-sst">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="bi bi-folder2-open me-2"></i>Gestión SST</h2>
    ...
    <p class="text-muted">Cree un documento SST (nombre, dependencia, imagen). Luego entre al detalle para agregar módulos y archivos dentro de cada módulo.</p>
    
    <div class="sst-contenido">
        <div class="row g-3">
                    ...
                    <div class="card shadow-sm h-100">
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex gap-1">
                                <a href="sst_detalle.php?id=<?php echo (int)$doc['id']; ?>" class="btn btn-primary btn-sm flex-grow-1">
    ...
    <?php if ($total_paginas > 1): ?>
        <nav class="sst-paginacion mt-auto" aria-label="Paginación de documentos">
        ...
        <div class="modal-content">
            <form method="post" enctype="multipart/form-data" id="formDocumento">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalDocumentoTitle">Nuevo documento SST</h5>
...
document.getElementById('modalDocumento').addEventListener('hidden.bs.modal', function() {
    ...
    document.getElementById('modalDocumentoTitle').textContent = 'Nuevo documento SST';
